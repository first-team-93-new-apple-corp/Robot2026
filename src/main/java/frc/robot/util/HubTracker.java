...
public class HubTracker {
    ...
    public static boolean isAutoWinner() {
        Optional<Alliance> autoWinner = getAutoWinner();
        return autoWinner.isPresent() && autoWinner.get() == DriverStation.getAlliance().get();
    }
    ...
    /**
     * Returns an Optional containing a countdown (in seconds) related to our
     * alliance's next active period.
     * - If the robot is not in a match (or alliance unknown), returns
     * Optional.empty().
     * - If our alliance is NOT currently active, returns a NEGATIVE value:
     * -(seconds until the next active period starts).
     * The value will reach 0 when our active period begins.
     * - If our alliance IS currently active, returns a POSITIVE value: (seconds
     * remaining until the current active period ends).
     * The value will reach 0 when the active period ends.
     */
    public static Optional<Integer> allianceActiveCountdownSeconds() {
        double matchTime = getMatchTime();
        if (matchTime < 0)
            return Optional.empty();
        ...
        // If we are currently in a shift and it's active for our alliance -> return
        // seconds remaining (positive)
        Optional<Shift> currentShift = getCurrentShift();
        if (currentShift.isPresent()) {
            Shift cs = currentShift.get();
            if (isActive(alliance, cs)) {
                return Optional.of((int)(cs.endTime - matchTime));
            }
        }

        // Not active now: find the next shift in the future that will be active for our
        // alliance.
        for (Shift s : Shift.values()) {
            if (matchTime < s.startTime && isActive(alliance, s)) {
                // Return negative seconds until that shift starts; it will reach 0 at the start
                // time.
                return Optional.of(-(int)(s.startTime - matchTime));
            }
        }

        // No future active period for our alliance found in this match
        return Optional.empty();
    }
