...
public class QuestNav {
  ...
  /**
   * Updates the drivetrain's pose estimator with the latest vision measurement from the Quest. Call this periodically to use the Quest for pose estimation.
    * This should be called in the RobotContainer periodic method, and the drivetrain should be passed in so that the vision measurement can be added to the drivetrain's pose estimator.
   *  @param drivetrain {@link CommandSwerveDrivetrain} the drivetrain to update with the vision measurement
   */
  public void updateVisionMeasurement(CommandSwerveDrivetrain drivetrain) {
    updateAverageRobotPose2D();
    updateAverageRobotPose3D();
    drivetrain.addVisionMeasurement(getRobotPose2D(), getTimestamp());
  }
  ...
  // TODO: Test this, generated by github copilot, may be wrong. my half baked
  // attempt is commented out below
  /**
   * Gets the raw pose of the Quest, relative to the position where it started
   *
   * @return pose of the Quest
